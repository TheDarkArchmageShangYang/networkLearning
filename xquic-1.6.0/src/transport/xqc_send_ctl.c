...
xqc_send_ctl_create(xqc_path_ctx_t *path)
{
    ...
    /* 初始化drop_rate测量*/
    send_ctl->drop_rate = xqc_calloc(1, sizeof(xqc_calc_drop_rate_node_t));
    // added by jndu
    xqc_log(conn->log, XQC_LOG_INFO, "|create drop rate|addr_of_drop_head:%xl|",&send_ctl->drop_rate->list_head);
    xqc_calc_drop_list_init(send_ctl->drop_rate);
...
xqc_send_ctl_destroy(xqc_send_ctl_t *send_ctl)
{
    ...
    // 删除drop_list所有节点
    // added by jndu
    xqc_calc_drop_list_del(send_ctl->drop_rate, send_ctl->drop_rate->list_head.prev);
    xqc_free(send_ctl->drop_rate);
}
...
xqc_send_ctl_on_ack_received(xqc_send_ctl_t *send_ctl, xqc_pn_ctl_t *pn_ctl, xqc_send_queue_t *send_queue, xqc_ack_info_t *const ack_info, xqc_usec_t ack_recv_time, xqc_bool_t ack_on_same_path)
{
    ...
    /* 测量throughout的变量 added by jndu*/
    uint16_t stream_frame_cnt = 0;
    uint64_t stream_frame_len = 0;
    float stream_frame_throughout = 0;
    xqc_calc_throughout_t power1;
    xqc_calc_throughout_reset(&power1);
    ...
    xqc_list_for_each_safe(pos, next, &send_queue->sndq_unacked_packets[pns]) {
        ...
        if (packet_out->po_pkt.pkt_num >= range->low) {
            ...
            xqc_calc_pkt_stream_size(packet_out, &stream_frame_len);
            stream_frame_throughout += xqc_calc_throughout(stream_frame_len, ack_recv_time - packet_out->po_sent_time);

            xqc_log(conn->log, XQC_LOG_INFO, "|xqc_calc_through|stream_frame_throughout %.5f|stream_frame_len %d|ack_recv_time %l|sent_time %l|",stream_frame_throughout, stream_frame_len, ack_recv_time, packet_out->po_sent_time);
            stream_frame_cnt++;
            // 修改标志位
            if (has_acked == 0) {
                /* 初始化 */
    ...
    /* 更新xqc_calc_throughout added by jndu*/
    xqc_calc_throughout_update(&power1,&(send_ctl->ctl_max_throughout), stream_frame_throughout, stream_frame_cnt);
    xqc_log(conn->log, XQC_LOG_INFO, "|xqc_calc_through_power1|cur %.5fMB/s|max %.5fMB/s|power1 %.5f|", power1.ct_cur_throughout, send_ctl->ctl_max_throughout, power1.ct_power1);
    printf("|xqc_calc_through_power1|cur %.5fMB/s|max %.5fMB/s|power1 %.5f|", power1.ct_cur_throughout, send_ctl->ctl_max_throughout, power1.ct_power1);
    /* TODO: ECN */
...
xqc_send_ctl_update_rtt(xqc_send_ctl_t *send_ctl, xqc_usec_t *latest_rtt, xqc_usec_t ack_delay)
{
    ...
    // Congestion switching CS added by wqn
    float power2 = *latest_rtt != 0 ? ((float)send_ctl->ctl_minrtt/(float)send_ctl->ctl_latest_rtt):1.0;
    xqc_log(send_ctl->ctl_conn->log, XQC_LOG_INFO, "|Congestion switching|minrtt:%uims|latest_rtt:%uims|power2:%.5f\n", send_ctl->ctl_minrtt, *latest_rtt, power2);
}
...
xqc_send_ctl_detect_lost(xqc_send_ctl_t *send_ctl, xqc_send_queue_t *send_queue, xqc_pkt_num_space_t pns, xqc_usec_t now)
{
    ...
    /* 测试drop_list的send部分 added by jndu*/
    int flag = 0;
    xqc_list_head_t *drop_pos = NULL, *drop_next = NULL;
    xqc_calc_drop_rate_node_t *drop = NULL;
    xqc_log(conn->log, XQC_LOG_INFO, "|jndu_test|send_ctl->drop_rate:%p", (void*)(&send_ctl->drop_rate->list_head));
    xqc_list_for_each_safe(drop_pos, drop_next, &send_ctl->drop_rate->list_head) {
        drop = xqc_list_entry(drop_pos, xqc_calc_drop_rate_node_t, list_head);
        xqc_log(conn->log, XQC_LOG_INFO, "|jndu_test|high:%d|low:%d|id=%d|", drop->drop_range.send_pktno_range.high, drop->drop_range.send_pktno_range.low, flag++);
    }
			...
			|| (lost_pn != XQC_MAX_UINT64_VALUE && po->po_pkt.pkt_num <= lost_pn))
		{
            ...
            if (po->po_flag & XQC_POF_IN_FLIGHT) {
                ...
                if(po->po_pkt.pkt_pns == XQC_PNS_APP_DATA) {
                    xqc_calc_drop_list_update(send_ctl, po->po_pkt.pkt_num);
                }
