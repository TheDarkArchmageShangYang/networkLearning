...
xqc_on_packets_send_burst(xqc_connection_t *conn, xqc_path_ctx_t *path, ssize_t sent, xqc_usec_t now, xqc_send_type_t send_type)
{
    ...
    // added by jndu
    if (low != -1) {
        // xqc_log(conn->log, XQC_LOG_INFO, "|jndu_test|low:%d|high:%d|",low , low + send_count - 1);
        xqc_calc_drop_list_add(send_ctl->drop_rate, low + send_count - 1, low);
    }
    ...
    xqc_list_head_t *head, int congest, xqc_send_type_t send_type)
{
    ...
    // added by jndu
    if(low != -1) {
        xqc_calc_drop_list_add(send_ctl->drop_rate, low + cnt - 1, low);
    }
